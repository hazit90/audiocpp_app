#!/usr/bin/env bash
#
# Makes a checkout ready to run on macOS, and is cheap enough to sit in front of
# every launch. Wired to VS Code's "Start Debugging" via .vscode/tasks.json, so
# a fresh clone reaches a working app with no manual steps.
#
# Does the minimum needed, in order:
#   1. initialises the audio.cpp submodule if it is empty
#   2. runs `flutter pub get` where it has never been run
#   3. rebuilds libaudiocpp_ffi.dylib only when it is missing or stale
#
...
DYLIB="${PACKAGE_DIR}/macos/Libs/libaudiocpp_ffi.dylib"
STAMP="${PACKAGE_DIR}/macos/Libs/.build-stamp"
...
needs_build() {
  ...
  [[ -f "${DYLIB}" ]] || { echo 'dylib missing'; return 0; }
  ...
  if [[ -n "$(find "${PACKAGE_DIR}/src" -type f -newer "${DYLIB}" -print -quit)" ]]; then
    echo 'shim sources changed'
    return 0
  fi

  if [[ "${PACKAGE_DIR}/tool/build_macos.sh" -nt "${DYLIB}" ]]; then
...
if reason="$(needs_build)"; then
  ...
  "${PACKAGE_DIR}/tool/build_macos.sh"
